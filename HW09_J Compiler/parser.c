...
void sexpr (FILE *theFile, map_elt **map)
{
  ...
  switch (current_token.type) {
    ...
    case IDENT:
      ...
      // take what was put in the stack an put it into the 
      // appropriate location
      fprintf(toWrite,
       "\n\t;; reversing stack for calling convention ;;\n");  

      for (i = arg_no; i > arg_no / 2; i--) {
        fprintf(toWrite, "\tLDR R2 R6 #%d\n", i - 1);
        fprintf(toWrite, "\tLDR R3 R6 #%d\n", arg_no - i);
        fprintf(toWrite, "\tSTR R2 R6 #%d\n", arg_no - i);
        fprintf(toWrite, "\tSTR R3 R6 #%d\n", i - 1);
      }
...
void function (FILE *theFile)
{
  ...
  while (current_token.type == IDENT) {
    ...
    put(map, current_token.str, 3 + arg_no);
    ++arg_no;
