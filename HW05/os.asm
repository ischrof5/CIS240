...
; the TRAP vector table
	...
	JMP BAD_TRAP	; x07
	JMP TRAP_PUTS		; x08
	JMP TRAP_WAIT_FOR_CHAR	; x09
	JMP TRAP_DRAW_RECT	; x0A
	JMP TRAP_DRAW_SEVEN_SEG	; x0B
	JMP BAD_TRAP	; x0C
	JMP BAD_TRAP	; x0D
	JMP BAD_TRAP	; x0E
	JMP BAD_TRAP	; x0F
...
;;; YOUR CODE HERE
	...
	LDR R1 R0 #0				; put that value from the address in R0 into R1
	BRz TRAP_PUTS_END	; if the value was zero, break (null termination)
	
	ADD R0 R0 #1				; increment the address for the next character
	
	LC R4, OS_ADDR_ADDR
	STR R1, R4, #0				; Write out the character
	JMP TRAP_PUTS		; loop back for next character
...
;;; TRAP_WAIT_FOR_CHAR - for getting a keyboard or timer event
;;; Input
;;;   none
;;; Output
;;;   r5 - keyboard character read
...
TRAP_WAIT_FOR_CHAR
	...
	LC R4, OS_KBSR_ADDR
	LDR R0, R4, #0		; Read the KBSR into R0
	BRzp TRAP_WAIT_FOR_CHAR		; Check if the MSB is zero

	; if it is, WAIT until it is one
...
;;; YOUR CODE HERE
;;; address of pixel vmem[y][x]: xC000 + (y*128) + x

	LC R6, OS_VIDEO_NUM_COLS
	CMPI R2 #0				; check for invalid inputs
	BRnz RECT_END		; negative dimension
	CMPI R3 #0
	BRnz RECT_END
	CMP R0 R6			; beginning pixel off allotted space
	BRzp RECT_END
	CMPIU R1 #124
	BRzp RECT_END
...
ERROR_CHECK
	...
	ADD R6 R1 R3			; check if the rectangle would go off the bottom of vmem
	CMPIU R6 #124
	BRnz RECT_INIT		; if it doesn't initialize the method
	ADD R3 R3 #-1		; if it does, decrement and check again
	JMP ERROR_CHECK
...
RECT_INIT 
	...
	ADD R6 R0 R2			; maximum column coordinate
	
	LC R1 OS_VIDEO_NUM_COLS	;for incrementing row

;;; draw a row

RECT_NEXT_COL
	CMP R0 R1			; edge case: rectangle overflows out side
	BRzp SKIP_PX 			; in edge case, skip next line, not drawing pixel
	STR R4, R5, #0		; Fill in the pixel
SKIP_PX
	ADD R5, R5, #1		; Update the pixel address
	ADD R0, R0, #1		; Update the column

	CMP R0, R6			; Test whether current column <= max column
	BRn RECT_NEXT_COL
	
;;; row is finished, change variables s.t. next row will be filled
	
	SUB R0 R0 R2				; Reset the column coordinate
	SUB R5 R5 R2				; Also reset the actual pointer
	ADD R5 R5 R1				; Next Row
	ADD R3 R3 #-1			; Decrement height counter
	BRp RECT_NEXT_COL	; continue to next row if more rows remain

RECT_END

	RTI
	
;;; TRAP_DRAW_SEVEN_SEG - draws an 8x8 sprite on the screen.
;;; Input
;;;   r0 - video column (left)
;;;   r1 - video row (upper) 
;;;   r2 - color
;;;   r3 - Lower seven bits indicate which segments to draw
;;; Output
...
;;; check for errors first (all pixels must be on screen)
...
;;; offset
	CONST R1 #4			; loop counter (segment is 4 px)
	ADD R6 R4 #1			; 1 across
	
;;;draw segment
LOOP_ZERO
	STR R2, R6, #0		; Fill in the pixel
	ADD R6, R6, #1		; Update the pixel address (right)
	
	ADD R1 R1 #-1		; decrement remaining count and loop if px remain
	BRp LOOP_ZERO
...
LOOP_ONE
	STR R2, R6, #0		; Fill in the pixel
	ADD R6, R6, R5		; Update the pixel address (down)
